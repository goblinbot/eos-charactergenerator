...
left join joomla.jml_eb_field_values v7 ON (v7.registrant_id = r.id AND v7.field_id = 94)
 join joomla.jml_eb_field_values v5 on (v5.registrant_id = r.id and v5.field_id = 14)
...
where ASCII(UPPER(LEFT(CONCAT(COALESCE(v4.field_value,''),COALESCE(v3.field_value,''),COALESCE(v8.field_value,'')),1))) BETWEEN 64 AND 90 
AND r.event_id = $EVENTID and ((v5.field_value != 'Speler' AND v7.field_value != 'No') or (v5.field_value = 'Keuken Crew' or v5.field_value = 'Spelleider')) 
and ((r.published = 1 AND (r.payment_method = 'os_ideal' OR r.payment_method = 'os_paypal' OR r.payment_method = 'os_bancontact')) OR 
...
left join joomla.jml_eb_field_values v7 on (v7.registrant_id = r.id and v7.field_id = 59)
left join joomla.jml_eb_field_values v8 on (v8.registrant_id = r.id and v8.field_id = 38)
where ASCII(UPPER(LEFT(CONCAT(COALESCE(v4.field_value,''),COALESCE(v3.field_value,''),COALESCE(v8.field_value,'')),1))) NOT BETWEEN 64 AND 90 
AND r.event_id = $EVENTID and ((v5.field_value != 'Speler' AND v7.field_value != 'No') or (v5.field_value = 'Keuken Crew' or v5.field_value = 'Spelleider')) 
and ((r.published = 1 AND (r.payment_method = 'os_ideal' OR r.payment_method = 'os_paypal' OR r.payment_method = 'os_bancontact')) OR 
(r.published in (0,1) AND r.payment_method = 'os_offline'))
UNION
